...
class FailureCaseChartGenerator:
    ...
    def create_gamestop_bull_view(self):
        """GameStop chart from bull perspective"""
        print("🟢 Creating GameStop bull interpretation...")
        ...
        # Create candlestick chart with our dark theme colors
    ...
    def create_gamestop_bear_view(self):
        """GameStop chart from bear perspective"""
        print("🔴 Creating GameStop bear interpretation...")
        ...
        fig, axes = mpf.plot(gme_plot_data, type='candle', style=style, volume=True,
                           figsize=(14, 10), returnfig=True,
                           title='BEAR VIEW: "Unsustainable Bubble"')
        
        ax_main = axes[0]
        
        # Highlight parabolic phase
        parabolic_start = pd.to_datetime('2021-01-25')
        peak_date = pd.to_datetime('2021-01-28')
        
        ax_main.axvspan(parabolic_start, peak_date, alpha=0.1, 
                       color=self.colors['accent_red'], label='Parabolic Exhaustion')
        
        if peak_date in gme_plot_data.index:
            peak_price = gme_plot_data.loc[peak_date, 'Close']
            ax_main.annotate('Gap ups = Distribution', 
                            xy=(peak_date, peak_price),
                            xytext=(-60, -50), textcoords='offset points',
                            fontsize=11, color=self.colors['accent_red'],
                            bbox=dict(boxstyle="round,pad=0.5", facecolor=self.colors['bg'], 
                                     edgecolor=self.colors['accent_red'], alpha=0.9),
                            arrowprops=dict(arrowstyle='->', color=self.colors['accent_red']))
        
        ax_main.text(0.02, 0.02, 'PATTERN: Parabolic Blowoff Top', 
                    transform=ax_main.transAxes, fontsize=12, color=self.colors['accent_red'], 
                    fontweight='bold', bbox=dict(boxstyle="round,pad=0.5", facecolor=self.colors['bg'], 
                                                edgecolor=self.colors['accent_red'], alpha=0.9))
        
        ax_main.legend(loc='upper left')
        plt.tight_layout()
        plt.savefig('assets/images/charts/gamestop_bear_view.png', 
                   format='png', dpi=150, bbox_inches='tight',
                   facecolor=self.colors['bg'], edgecolor='none')
        plt.close()
        
    def create_netflix_views(self):
        """Create both Netflix fundamental and market reality views"""
        print("📺 Creating Netflix perspective charts...")
        ...
        style = self.create_base_style()
        earnings_date = pd.to_datetime('2022-04-19')
        pre_period = pd.to_datetime('2022-03-01')
        
        # Fundamental view
        fig, axes = mpf.plot(nflx_plot_data, type='candle', style=style, volume=True,
                           figsize=(14, 10), returnfig=True,
                           title='FUNDAMENTAL VIEW: "All Systems Go!" (Pre-Earnings)')
        
        ax_main = axes[0]
        ax_main.axvspan(pre_period, earnings_date, alpha=0.1, 
                       color=self.colors['accent_blue'], label='Strong Fundamentals Period')
        ax_main.axvline(x=earnings_date, color=self.colors['accent_blue'], 
                       linestyle='--', linewidth=2, alpha=0.8, label='Earnings Date')
        
        ax_main.text(0.02, 0.85, 'STRONG Q4 2021 METRICS:\n• Revenue: 7.7B (+16% YoY)\n• 8.3M net subscriber adds\n• Operating margin: 24.3%\n• Content spend: 17B planned', 
                    transform=ax_main.transAxes, fontsize=10, color=self.colors['accent_blue'],
                    bbox=dict(boxstyle="round,pad=0.7", facecolor=self.colors['bg'], 
                             edgecolor=self.colors['accent_blue'], alpha=0.9))
        
        ax_main.legend(loc='upper right')
        plt.tight_layout()
        plt.savefig('assets/images/charts/netflix_fundamentals_view.png', 
                   format='png', dpi=150, bbox_inches='tight',
                   facecolor=self.colors['bg'], edgecolor='none')
        plt.close()
        ...
        fig, axes = mpf.plot(nflx_plot_data, type='candle', style=style, volume=True,
                           figsize=(14, 10), returnfig=True,
                           title='MARKET REALITY: "Fundamentals Don\'t Matter!"')
        
        ax_main = axes[0]
        ax_main.axvline(x=earnings_date, color=self.colors['accent_red'], 
                       linestyle='-', linewidth=3, alpha=0.9, label='Earnings Shock')
        
        post_period_end = pd.to_datetime('2022-05-15')
        ax_main.axvspan(earnings_date, post_period_end, alpha=0.1, 
                       color=self.colors['accent_red'], label='Post-Earnings Reality')
        
        ax_main.text(0.55, 0.85, 'WHAT MARKET ACTUALLY CARED ABOUT:\n• Q1 subscriber LOSS: -200K\n• Q2 guidance: -2M subscribers\n• "Peak streaming" narrative\n• Rising rates hurt growth stocks', 
                    transform=ax_main.transAxes, fontsize=10, color=self.colors['accent_red'],
                    bbox=dict(boxstyle="round,pad=0.7", facecolor=self.colors['bg'], 
                             edgecolor=self.colors['accent_red'], alpha=0.9))
        
        ax_main.legend(loc='upper left')
        plt.tight_layout()
        plt.savefig('assets/images/charts/netflix_market_reality.png', 
                   format='png', dpi=150, bbox_inches='tight',
                   facecolor=self.colors['bg'], edgecolor='none')
        plt.close()
    ...
    def generate_all_charts(self) -> dict:
        ...
        # Generate detailed perspective charts
        self.create_gamestop_bull_view()
        self.create_gamestop_bear_view()
        self.create_netflix_views()
