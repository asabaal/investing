...
def create_phase_space_density_gradient(ohlc_data: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(go.Heatmap(
        ...
        showscale=True,
        colorbar=dict(
            title="Candle<br>Density",
            titlefont=dict(color='white', size=14),
            tickfont=dict(color='white'),
            x=1.02
        ),
        hovertemplate='Sentiment: %{x:.2f}<br>UWR: %{y:.2f}<br>Density: %{z:.4f}<extra></extra>',
        name='Density Field'
    ))
    ...
    # Update layout
    fig.update_layout(
        title='🎯 Phase Space Density Gradient<br><sub>Candle Clustering Patterns (No Individual Particles)</sub>',
        xaxis=dict(
            title='← Bearish | Sentiment | Bullish →',
            range=[-1, 1],
...
def create_interactive_combined_analysis(ohlc_data: pd.DataFrame) -> go.Figure:
    ...
    """
    print("🌌 Creating interactive combined gradient analysis...")
    ...
    # Create subplots with 2.5 columns (no bottom-right)
    fig = make_subplots(
        rows=2, cols=3,
        subplot_titles=(
            'Density Gradient',
    ...
    fig.add_trace(
        go.Contour(
            ...
            contours=dict(
                start=0,
                end=np.nanmax(density_grid),
                size=np.nanmax(density_grid)/8
            )
        ),
        row=1, col=1
    )
    ...
    # Add boundaries to phase space subplots only (not candlestick or 3D)
    boundary_s = np.linspace(-0.99, 0.99, 50)
    boundary_u = 1.0 - np.abs(boundary_s)
    
    # Only add boundaries to phase space plots (cols 1,2 and row 2 col 2)
    ...
    # Update all phase space axes (including trajectory)
    phase_space_plots = [(1,1), (1,2), (2,1), (2,2)]
    for row, col in phase_space_plots:
